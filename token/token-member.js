...
class JWT {
  ...
  checkToken () {
    process.env.TOKENS_MEMBER = 'tokenmember46786djfdsklmfkmdlscsq'

    return (req, res, next) => {
      if (req.headers['member-auth'] || req.headers['token']) {
        const token = req.headers['member-auth'] || req.headers['token']
